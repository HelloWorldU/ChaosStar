...
# Google
google_engine = GenericWebSearchEngine(
    name="Google",
    search_fn=lambda q, n: google_search(q, num_results=n, advanced=True),
)

# Baidu
baidu_engine = GenericWebSearchEngine(
    name="Baidu",
    search_fn=lambda q, n: baidu_search(q, num_results=n),
    title_field="title",
    url_field="url",
    desc_field="abstract",
)

proxy_config = next(iter(config.llm.values())).proxy if config.llm else None

def _is_relevant_redirect(original_url: str, redirected_url: str) -> bool:
    """Check if the redirect is relevant by comparing domains."""
    from urllib.parse import urlparse
    orig_domain = urlparse(original_url).netloc
    redir_domain = urlparse(redirected_url).netloc
    return orig_domain == redir_domain
    ...
    def populate_output(self) -> "SearchResponse":
        ...
        for i, result in enumerate(self.results, 1):
            ...
            if result.raw_content:
                logger.info(f"Content preview for {result.url}: {result.raw_content[:1000]}...")
...
class WebContentFetcher:
    ...
    @staticmethod
    async def fetch_content(url: str, timeout: int = 10, proxy: str = proxy_config) -> Optional[str]:
        """
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.get(url, headers=headers, timeout=timeout, proxies=proxies)
            )
            ...
            # Check for redirect
            if response.url != url and not _is_relevant_redirect(url, response.url):
                logger.warning(f"URL {url} redirected to unrelated page: {response.url}")
                return None
    ...
    async def _fetch_single_result_content(self, result: SearchResult) -> SearchResult:
        ...
        if result.url:
            ...
            if content:
                ...
                result.raw_content = "Failed to fetch content, redirecting to unknown URL."
        return result
